...
const tempAccessEl = document.getElementById("temp-access-badge");
const tempAccessBannerEl = document.getElementById("temp-access-banner");
let tempAccessInterval = null;
...
function updateTempAccessBadge() {
  if (!tempAccessEl) return;
  const until = parseInt(localStorage.getItem('chatAccessUntil') || '0', 10);
  if (until && until > Date.now()) {
    tempAccessEl.style.display = 'inline-block';
    tempAccessEl.textContent = `Temporary access · ${formatRemaining(until - Date.now())}`;
    if (tempAccessBannerEl) {
      tempAccessBannerEl.style.display = 'block';
      tempAccessBannerEl.textContent = `Temporary access — expires in ${formatRemaining(until - Date.now())}`;
    }
    if (!tempAccessInterval) tempAccessInterval = setInterval(updateTempAccessBadge, 1000);
  } else {
    tempAccessEl.style.display = 'none';
    if (tempAccessBannerEl) tempAccessBannerEl.style.display = 'none';
    if (tempAccessInterval) { clearInterval(tempAccessInterval); tempAccessInterval = null; }
    try { localStorage.removeItem('chatAccessUntil'); } catch (e) {}
  }
}
...
/* ---------------------------------------------------------
   ...
   Keeps the latest message visible when the on-screen keyboard
   opens/closes or the input is focused, instead of leaving the
   view stuck mid-scroll (a common issue on phones).
--------------------------------------------------------- */
if (inputEl) {
  ...
  contactOverlay.innerHTML = `
    <div class="modal" style="max-width:400px;">
      ...
      <p style="font-size:14px; margin-bottom:22px;">
        📧 <a href="mailto:hussain@gmail.com" style="color:var(--primary); font-weight:500;">hussain@gmail.com</a>
      </p>
      <div class="modal-actions">
...
// Initialize temporary access badge if present
try { updateTempAccessBadge(); } catch (e) {}
...
function renderModeMenu() {
  ...
  document.querySelectorAll(".mode-menu-item").forEach((item) => {
    ...
    const locked = !isPaid() && item.dataset.mode !== "general";
    item.disabled = false; // keep clickable so a click can open the upgrade modal
    item.classList.toggle("selected", isActive);
    item.classList.toggle("locked-mode", locked);
    item.title = locked ? "Pro feature — upgrade to unlock" : "";
...
modeMenu?.addEventListener("click", (e) => {
  ...
  if (!isPaid() && item.dataset.mode !== "general") {
    closeModeMenu();
    openModal(upgradeModal);
    return;
  }
...
modeBar.addEventListener("click", (e) => {
  ...
  if (!pill) return;
  const chat = getActiveChat();
  if (!isPaid() && pill.dataset.mode !== "general") {
    openModal(upgradeModal);
    return;
  }
